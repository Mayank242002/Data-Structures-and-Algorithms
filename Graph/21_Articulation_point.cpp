...
void dfs(int node,int parent,vector<int> &vis,vector<int> &in,vector<int> &low,int &timer,vector<int> adj[],vector<bool> &ap)
{
    vis[node]=1;
    ...
    for (auto it:adj[node])
    {
        ...
        else
        {
            ...
            if (low[it]>=in[node] && parent!=-1)   //(it would be only possible if there is only way to react child node)
                ap[node]=true;
        }
    }
...
int main()
{
    ...
    for (int i=1;i<=n;i++)
    {
        if (!vis[i])
        {
            dfs(i,-1,vis,in,low,timer,adj,ap);
        }
    }
    int child_of_root=adj[1].size();
    if (child_of_root>1) ap[1]=true;
    for (int i=1;i<=n;i++)
    {
        if (ap[i])
          cout<<i<<", ";
    }
    cout<<"are articulation points"<<endl;
